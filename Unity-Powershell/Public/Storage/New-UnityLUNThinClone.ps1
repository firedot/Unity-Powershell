Function New-UnityLUNThinClone {
  ...
  Begin {
    ...
    ## Variables
    $URI = '/api/instances/storageResource/<id>/action/createLunThinClone'
    $Type = 'ThinClone'
    $StatusCode = 200
  }
  ...
  Process {
    Foreach ($sess in $session) {
      ...
      If ($Sess.TestConnection()) {
        ...
        #Sending the request
        If ($pscmdlet.ShouldProcess($Sess.Name,"Create $Type on  LUN ID $ObjectID")) {
        ...
        If ($request.StatusCode -eq $StatusCode) {
          ...
          Get-UnityLUN -Session $Sess | Where-Object {$_.storageResource.id -eq $results.storageResource.id}
